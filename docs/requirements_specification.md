...
### 4.1 アーキテクチャパターン
クリーンアーキテクチャに基づいた3層構造を採用

```mermaid
graph TB
    Client["クライアント"] --> Handler["Handler層"]
    Handler --> Service["Service層"]
...
├── route.go                # ルーティング設定
├── handler/                # HTTPハンドラー層
│   └── attendance.go       # 出勤管理ハンドラー
├── service/                # ビジネスロジック層
│   ├── attendance.go       # 出勤管理サービス
│   └── repository/         # リポジトリインターフェース
│       └── attendance.go   # 出勤データモデル・インターフェース
├── database/               # データアクセス層
│   ├── database.go         # DB接続管理
│   └── attendance.go       # 出勤リポジトリ実装
├── validator/              # バリデーション
│   └── validator.go        # カスタムバリデーター
├── logger/                 # ロギング
│   └── logger.go           # ロガー設定
├── myerrors/               # エラーハンドリング
│   └── myerrors.go         # カスタムエラー定義
└── build/                  # ビルド・環境設定
    ├── Dockerfile
    └── database/
        └── sql/
            ├── 01_create.sql   # テーブル作成
            └── 02_insert.sql   # 初期データ投入
```
